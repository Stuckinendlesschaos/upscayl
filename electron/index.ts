...
app.on('ready', async () => {
  ...
  mainWindow.webContents
    .executeJavaScript('localStorage.getItem("lastCustomModelsFolderPath");', true)
...
ipcMain.on(commands.GET_MODELS_LIST, async (event, payload) => {
  if (payload) {
    ...
    mainWindow.webContents.send(commands.CUSTOM_MODEL_FILES_LIST, getModels(payload))
  }
})
...
ipcMain.handle(commands.SELECT_CUSTOM_MODEL_FOLDER, async (event, message) => {
  ...
  if (canceled) {
    ...
    if (!folderPaths[0].endsWith('/models') && !folderPaths[0].endsWith('/models/')) {
    ...
    mainWindow.webContents.send(commands.CUSTOM_MODEL_FILES_LIST, getModels(customModelsFolderPath))
...
ipcMain.on(commands.DOUBLE_UPSCAYL, async (event, payload) => {
  ...
  const outFile = outputDir + slash + fileName + '_upscayl_16x_' + model + '.' + saveImageAs
...
ipcMain.on(commands.UPSCAYL, async (event, payload) => {
  ...
  const outFile =
    outputDir + slash + fileName + '_upscayl_' + scale + 'x_' + model + '.' + saveImageAs
  ...
  if (fs.existsSync(outFile)) {
    ...
    const onClose = () => {
      if (!failed && !stopped) {
        ...
        mainWindow.webContents.send(commands.UPSCAYL_DONE, isAlpha ? outFile + '.png' : outFile)
      }
    }
...
ipcMain.on(commands.FOLDER_UPSCAYL, async (event, payload) => {
  ...
  const onData = (data: any) => {
    data = data.toString()
    mainWindow.webContents.send(commands.FOLDER_UPSCAYL_PROGRESS, data.toString())
    if (data.includes('invalid gpu') || data.includes('failed')) {
      logit('❌ INVALID GPU OR FAILED')
      failed = true
    }
