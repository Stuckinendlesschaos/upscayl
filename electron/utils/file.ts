...
export const getAddBgImgData = (payload) => {
  return new Promise((resolve, reject) => {
    const data = new FormData()
    data.append('prompt', payload.prompt)
    data.append('negativePrompt', payload.negativePrompt)
    data.append('seed', payload.seed)
    data.append('imageFile', fs.createReadStream(payload.imagePath))
    const config = {
      method: 'post',
