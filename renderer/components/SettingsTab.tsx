...
function SettingsTab({
  ...
  useEffect(() => {
    ...
    if (!localStorage.getItem('saveImageAs')) {
      ...
      logit('📢 Getting saveImageAs from localStorage', currentlySavedImageFormat)
    ...
    if (!localStorage.getItem('model')) {
      ...
      logit('📢 Getting model from localStorage', JSON.stringify(currentlySavedModel))
    }
    ...
    if (!localStorage.getItem('rememberOutputFolder')) {
      ...
      logit('📢 Getting rememberOutputFolder from localStorage', currentlySavedRememberOutputFolder)
    ...
    <div className="animate-step-in animate flex h-screen flex-col gap-7 overflow-y-auto p-5 overflow-x-hidden">
      ...
      <div className="flex flex-col gap-2">
        ...
        <input
          ...
          onClick={() => {
            ...
            localStorage.setItem('rememberOutputFolder', JSON.stringify(!rememberOutputFolder))
          }}
        />
      </div>
      ...
      <div className="flex flex-col gap-2">
        <div className="flex flex-row gap-1">
          ...
          <p className="badge-primary badge text-[10px] font-medium">测试中</p>
        </div>
        <div className="flex flex-col gap-2">
          {batchMode && (
            <p className="text-xs text-base-content/70">Only PNG is supported in Batch Upscale</p>
          )}
          <div className="flex flex-wrap gap-2">
            {/* PNG */}
            <button
              className={`btn-primary btn ${saveImageAs === 'png' && 'btn-accent'}`}
            ...
            {/* JPG */}
            <button
              className={`btn-primary btn ${saveImageAs === 'jpg' && 'btn-accent'}`}
            ...
            {/* WEBP */}
            <button
              className={`btn-primary btn ${saveImageAs === 'webp' && 'btn-accent'}`}
